...
- `backup.py` / `retention.py` — nightly cron jobs (registered in main.py lifespan): JSON backup with rotation (`BACKUP_KEEP`), price-history thinning to daily min/max (`PRICE_HISTORY_THIN_AFTER_DAYS`, 0 = off), and event-log pruning (`EVENT_LOG_RETENTION_DAYS`, 0 = keep). `build_export_payload` there is the single source for both the export API and backups — extend it when models change.
...
**Frontend** (`frontend/src/`) — pages in `pages/` (Dashboard, ProductDetail, AddProduct, Login, Setup), section components in `components/product-detail/` and `components/setup/`, API layer in `api/` (`client.ts` fetch wrapper with Bearer token from localStorage, `hooks.ts` TanStack Query hooks, `types.ts`). Display formats come from `useUiSettings` (all users); `useSettings`/`useSaveSettings` are admin-only. Settings tabs (Account · Schedulers · Logs · Admin · About) live in `?tab=`; Logs filters live in the URL too (`utils/logFilters.ts`). Charts via Recharts (route-split so the dashboard chunk stays lean — dashboard sparklines use the tiny SVG `Sparkline` component instead), routing via react-router-dom.
