...
- `scheduler.py` — one APScheduler job per active product. Scrape job records price history (only when the price changed, unless the product's `record_all_prices` toggle is on), detects URL redirects, and triggers alerts (`evaluate_alert` is the pure condition logic; "below" fires only when *crossing* the threshold). The scrape itself runs between two short DB sessions — never hold a session across a scrape. Single-process only — never add uvicorn workers or jobs run multiple times. Jobs carry a `misfire_grace_time` (`SCRAPE_MISFIRE_GRACE_SECONDS`) plus `coalesce` so a late scrape is deferred rather than dropped. Scraper health is global module state (`scraping_looks_broken`, `last_successful_scrape_at`, `reset_scrape_health` for tests): once `SCRAPE_STORM_MIN_PRODUCTS` distinct products fail with no success between, per-product "selector broken" mail gives way to one outage message per user, because failures that widespread are infrastructure, not selectors. `run_check_all` runs one full "Check all" pass at a time. `scrape_and_record` returns the outcome (`ok`/`unchanged`/`failed`/`skipped`). Nightly jobs are declared once in `MAINTENANCE_JOBS`. An APScheduler listener logs missed/errored runs. Admin "Run now" (`POST /api/settings/jobs/{id}/run`) adds a one-off `<id>__run_now` date job — never `modify_job(next_run_time=…)`, which would re-anchor the interval and drift the product off its check time.
- `scraper.py` — Patchright-based scraping; concurrency capped by a semaphore (`SCRAPER_CONCURRENCY`) and each scrape bounded end-to-end by `SCRAPE_TIMEOUT_SECONDS` so a wedged browser cannot hold its slot forever. A scrape without a price carries a `FAILURE_*` reason code (timeout, page error, unavailable, selector matched nothing, unparseable text); the scheduler stores it with the streak start (`last_scrape_error`, `scrape_failing_since`) so the UI can say whether the selector is the likely fix. `ScrapeResult.source` (`selector`/`ld_json`/`itemprop`/`data_price`) records which strategy found the price and `error_detail` the exception text of a page error; both land in the admin event log. The UI stays quiet below `SCRAPER_FAILURE_ALERT_THRESHOLD` (sent via `/api/settings/ui`) and stops blaming selectors while `/api/health` reports `scraping_degraded`. Concurrency is also a memory knob: each parallel scrape adds a browser context and renderer on top of the one shared Chromium, which under `SCRAPER_HEADLESS=false` runs inside xvfb. Pure text parsing (price normalization for European/English formats with a per-product `price_format` override, currency detection) lives in `parsing.py` so it is importable without Patchright. Scheduled runs get a random offset whose window scales with how many products share a check time (`SCHEDULE_JITTER_SECONDS` as floor, `SCHEDULE_JITTER_PER_PRODUCT_SECONDS` per product), so the spread doesn't degrade as products are added. Notifications retry twice with backoff. Login rate limiting relies on `--proxy-headers` + `FORWARDED_ALLOW_IPS` behind a reverse proxy.
- `stats.py` — `time_weighted_average`; prices are stored on change only, so averages must weight by time-in-effect, not by row.
- `events.py` — admin event log (Settings → Logs), table `event_log`. `log_event(db, …)` joins the caller's session (the scrape path, so events commit with the writes they describe); `record_event`/`spawn_event` open their own session and never raise. `details` is JSON-made-safe (Decimals → strings). `message` and every string in `details` pass through `notifier._redact` (lazily imported to avoid a circular import), so secrets can't reach persisted events. A `before_delete` listener on `Product` nulls `event_log.product_id` (SQLite has no FK enforcement) — bulk deletes (e.g. import) must do this themselves since they bypass the ORM listener. Pruned nightly by `EVENT_LOG_RETENTION_DAYS` (default 30, 0 = keep) in `retention.run_nightly_retention`; deliberately excluded from backup/export. Scrape events carry the `FAILURE_*` reason, `source` and `error_detail` from `ScrapeResult`.
- `routers/` — API under `/api`: `auth`, `products`, `prices`, `alerts`, `settings`. Full app settings are admin-only and never return secrets (`telegram_bot_token_set` boolean instead; `""` = clear). Save with `PATCH /api/settings` (only the sent fields change; explicit `null` resets a keep knob to its env value); `POST` is the legacy full save. `/api/settings/ui` exposes display preferences (date/time format, dashboard sparklines, chart line style) to all users. Admin-only `/api/settings/jobs` (enriched with last result from the event log) and `/api/settings/logs` (keyset-paged via `before_id`).
- `models.py` — `User`, `Product`, `PriceHistory`, `Alert`, `SelectorDefault`, `AppSettings`, `EventLog`. DB connection config comes from `.env` only, not `AppSettings`.
- `notifier.py` — Telegram + SMTP per-recipient, plus global webhook broadcast channels (ntfy/Gotify/Discord via `NTFY_URL`, `GOTIFY_URL`+`GOTIFY_TOKEN`, `DISCORD_WEBHOOK_URL` — every notification goes to each configured one). `notify()` is the single dispatch helper and takes a structured `Notification` (title/product/facts/text/links), rendered per channel: Telegram HTML (names escaped; inline link buttons only for real domains, plain-text resend on a 400), plain text for email/ntfy/Gotify, markdown for Discord. `send_alert()` builds price-alert messages on top of it. The public URL for "Open in Caero" links (`product_links`/`caero_url`) and the Telegram bot token both come from env (`PUBLIC_URL`, `TELEGRAM_BOT_TOKEN`) or from `AppSettings` in the DB (DB wins; no URL means links are omitted). Delivery health is in-memory per channel (`channel_statuses`, `reset_channel_status` for tests) and shown in Settings; error text passes through `_redact` because httpx errors embed the bot token in the URL.
...
- `backup.py` / `retention.py` / `maintenance.py` — nightly cron jobs: JSON backup with rotation, price-history thinning to daily min/max, and event-log pruning. `maintenance.MaintenanceConfig` is the one validated view of their settings: on/off and HH:MM run time per job from `AppSettings`, plus the keep knobs (`backup_keep`, `price_history_thin_after_days`, `event_log_retention_days`) where a NULL column means the env var of the same name applies (0 = off). The jobs read the knobs at run time; an invalid stored time falls back to the default instead of failing startup. `scheduler.apply_maintenance_schedule` (re)schedules them with a misfire grace + coalesce; the settings endpoints and import go through `apply_maintenance_schedule_after_commit` so the scheduler only changes once the DB has. A disabled job is paused, not removed, so "Run now" still works; every change is logged as a `maintenance_settings` event. `build_export_payload` there is the single source for both the export API and backups — extend it when models change.
- Product check times (`check_time_hhmm`) use the container's local timezone — set `TZ` in `.env`.

**Transactions:** `get_db` commits once on success and rolls back on error. Endpoints mutate and at most `flush()`/`refresh()`; they do not call `commit()` (background jobs with their own sessions do). Always inject it as `Depends(get_db, scope="function")`: FastAPI's default request scope runs the commit *after* the response is sent, so a client acting on a 201 right away can 404 on the row it just created (`tests/test_commit_before_response.py`).
...
**Frontend** (`frontend/src/`) — pages in `pages/` (Dashboard, ProductDetail, AddProduct, Login, Setup), section components in `components/product-detail/` and `components/setup/`, API layer in `api/` (`client.ts` fetch wrapper with Bearer token from localStorage, `hooks.ts` TanStack Query hooks, `types.ts`). Display formats come from `useUiSettings` (all users); `useSettings`/`useSaveSettings` (PATCH, send only the changed fields) are admin-only. Maintenance on/off, times and keep knobs are edited inline on the Schedulers tab (`MaintenanceControls`); schedule labels are formatted client-side (`describeSchedule`) for the 12h/24h preference. Settings tabs (Account · Schedulers · Logs · Admin · About) live in `?tab=`; Logs filters live in the URL too (`utils/logFilters.ts`). Charts via Recharts (route-split so the dashboard chunk stays lean — dashboard sparklines use the tiny SVG `Sparkline` component instead), routing via react-router-dom.
